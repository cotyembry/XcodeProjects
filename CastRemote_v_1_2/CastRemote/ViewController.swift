...
/*
    ...
    TODO: add another labelView (maybe) to take into account the "no media playing to skip to" option since i stole that label view for the media playing label
    ...
    TODO: make sure that I clear the [Actions] array before adding the devices so that it will correctly reflect when a device goes offline a user can't select the device to try to connect to it (done I think)
...
//        navigationItem.rightBarButtonItems[0].image = UIBarButtonItem.i
        ...
        labelMediaTitleView.numberOfLines = 0
            ...
            if castInstance!.deviceDidGoOffline == false {
                ...
                dispatch_async(dispatch_get_global_queue(priority, 0)) {
                    ...
                    dispatch_async(dispatch_get_main_queue()) {
                        ...
                        self.navigationItem.rightBarButtonItems![0].image = UIImage(named: "ic_cast_connected_black_24dp.png")
                        self.castButton!.title = "Connected to \(castInstance?.myDevice?.friendlyName)"
                    }
                }
            }
...
//        print("season number is? => \(testingTry.stringForKey(kGCKMetadataKeySeasonNumber )) ")
    ...
    @IBAction func castButtonViewAction(sender: UIBarButtonItem) {
        //this first line just clears the action buttons incase one is missing from the past (i.e. a device went offline)
        castButton = nil
