...
class ViewController: UIViewController, UITextFieldDelegate, UpdateView, ApplicationStatus, ADBannerViewDelegate {
    ...
    var connectToDeviceHelper = ConnectOnStartup()
    ...
    //= UIAlertController.init(title: "Chromecast Devices", message: "Select To Connect", preferredStyle: .ActionSheet)
...
//        navigationItem.rightBarButtonItems[0].image = UIBarButtonItem.i
        ...
        //I have to create a new thred to get into a new flow then have to call the main thread to start scanning for the devices in range. If I didn't do the new thread, this wouldn't work for some reason
        let queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)
        dispatch_async(queue) {
            self.connectToRunningChromecast() //this is to connect to a chromecast if there is one that is already playing media as soon as the app is launched
        }
...
//            deviceScanner.stopScan()
    ...
    //I use this as a convienence to connect to a chromecast if there is media playing on one already
    //I'm not sure about how to decide which device to connect to if two devices are busy yet
    //Maybe some sort of preference setting, or a "frequency" metric and make it persistent data
    func connectToRunningChromecast() {
        //TODO
        //Improve this because this might not catch the chromecast for slow internet connections
        sleep(1)
        var devicesConnected = [GCKDevice]()
        
        //to start scanning again, I have to do this on the main thread
        if let deviceScanner = castInstance!.deviceScanner {
            let priority = DISPATCH_QUEUE_PRIORITY_DEFAULT
            dispatch_async(dispatch_get_global_queue(priority, 0)) {
                // do some task
                dispatch_async(dispatch_get_main_queue()) {
                    deviceScanner.startScan()
                    deviceScanner.passiveScan = false
                    var i = 0 //I'll use this to help with getting the correct device to connect to later
                    for device in deviceScanner.devices  {
                        if ((device.status == GCKDeviceStatus.Busy)) {
                            devicesConnected.append(device as! GCKDevice)
                        }
                        i++
                    }
                    deviceScanner.passiveScan = true
                    
                    //print("testing: \(devicesConnected.count)")
                    if devicesConnected.count >= 1 {
                        self.connectToDevice(devicesConnected[0])
                        
                        
                    }
                    //castInstance!.myDevice = devicesConnected[0] as! GCKDevice //this is necesary to be used in the castInstance class so the connection can be made correctly
                    //castInstance!.deviceManager!.connect()
