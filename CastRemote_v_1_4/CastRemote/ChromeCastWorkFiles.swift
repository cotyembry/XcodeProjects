...
class ChromeCastWorkFiles: NSObject, GCKDeviceScannerListener, GCKDeviceManagerDelegate, GCKMediaControlChannelDelegate {
    ...
    func deviceDidGoOffline(device: GCKDevice!) {
        ...
        //if the device goes offline that I'm currently connected to I want to update the image to show the app is no longer connected
        delegate?.updateUIBarButtonImage(device)
        delegate?.updateStreamDuration()
        ...
        deviceDidGoOffline = true
                print("device \(device.friendlyName) went offline")
